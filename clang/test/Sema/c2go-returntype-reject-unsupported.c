// c2go_returntype(struct) is rejected unless the struct's field is directly
...
struct A1 { int *p; };
extern struct A1 A1f(int) c2go_returntype(struct A1);

struct A2 { float f; double d; };
extern struct A2 A2f(int) c2go_returntype(struct A2);

struct A3 { c2go_slice s; };
extern struct A3 A3f(int) c2go_returntype(struct A3);
