...
// the addrspace(1) GC discriminator, so Sema warns (it is allowed but lossy). The escape hatch is an
...
void f(struct N *managed) {
  ...
  void *p;
  p = managed; // expected-warning{{allowed but lossy}}

  // Declaration initialization is also a store.
  void *q = managed; // expected-warning{{allowed but lossy}}
  (void)q;
  ...
  void *arr[1] = { managed }; // expected-warning{{allowed but lossy}}
  (void)arr;

  struct U { void *p; };
  struct U u = { managed }; // expected-warning{{allowed but lossy}}
  (void)u;
}
