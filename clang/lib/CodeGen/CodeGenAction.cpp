...
static llvm::json::Object buildC2GoManifest(ASTContext &Ctx,
  ...
  // Path-(b) linkname bridges. Cross-package ABIInternal functions and target
  // spellings that Plan 9 cannot carry use a sanitised local stub generated by
  // c2go-bind. Same-package targets use their local suffix and therefore need
  // no bridge when that suffix is directly representable; Go supplies the ABI0
  // wrapper for a same-package ABIInternal Go function.
  ...
  for (const Decl *D : Ctx.getTranslationUnitDecl()->decls()) {
    ...
    std::optional<StringRef> LocalName =
        llvm::c2go::getC2GoSamePackageSymbol(Target, LangOpts.C2GoPackagePath);
    StringRef EmittedName = LocalName ? *LocalName : Target;
    bool Plan9Direct = LocalName
                           ? llvm::c2go::isC2GoPlan9LocalSymbol(EmittedName)
                           : llvm::c2go::isC2GoPlan9PathSymbol(EmittedName);
    // Direct (path a): clean same-package targets, clean variables (data has no
    // ABI), and cross-package functions explicitly marked C2GO_GOABI0.
    bool VarKind = isa<VarDecl>(D);
    bool Direct = Plan9Direct &&
                  (LocalName.has_value() || VarKind || LA->getHasAbi0() != 0);
    ...
    Bridge["asm_symbol"] =
        "\xc2\xb7" + llvm::c2go::sanitiseC2GoSymbolToIdent(Target);
