...
static void addC2GoEarlyPasses(llvm::ModulePassManager &MPM, bool UseStatepoint,
                               bool DeferLatePipeline) {
  MPM.addPass(C2GoMemcpyTypingPass());
  if (!UseStatepoint && !DeferLatePipeline)
...
#include "llvm/Support/Extension.def"
  ...
  if (!CodeGenOpts.DisableLLVMPasses) {
    ...
    // managed allocas. In c2go-lto pre-link mode only MemcpyTyping runs here;
    // safepoint/GC lowering is deferred until after whole-module inlining.
    if (LangOpts.C2GoMode)
      PB.registerPipelineStartEPCallback(
          [DeferLatePipeline = CodeGenOpts.C2GoLTOPreLink](
              ModulePassManager &MPM, OptimizationLevel Level) {
            bool UseStatepoint =
                !llvm::c2go::isC2GoDisabled("statepoint-gc") &&
                Level.getSpeedupLevel() >= 2;
            addC2GoEarlyPasses(MPM, UseStatepoint, DeferLatePipeline);
          });
    ...
    // intrinsics, so OptimizerLast captures them all. The c2go-lto route skips
    // this callback: its bitcode is explicitly pre-link IR, and c2go-lto runs
    // this exact shared sequence after linking and inlining.
    if (LangOpts.C2GoMode && !CodeGenOpts.C2GoLTOPreLink)
      PB.registerOptimizerLastEPCallback(
          [](ModulePassManager &MPM, OptimizationLevel Level,
             ThinOrFullLTOPhase) {
            bool UseStatepoint =
