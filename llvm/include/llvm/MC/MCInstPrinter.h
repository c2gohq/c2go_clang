...
public:
  ...
  /// Return the c2go Plan 9 symbolic-printer adapter for this
  /// InstPrinter, or nullptr if the printer doesn't support Plan 9
  /// dispatch. AArch64Plan9InstPrinter overrides this to return `this`;
  /// all other InstPrinters return nullptr. Used by MCPlan9AsmStreamer
  /// to call into the target-specific tryPrintInst without LLVM-style
  /// RTTI (LLVM core is built with `-fno-rtti`) and without lib/MC
  /// having to know about per-target headers.
  virtual MCPlan9SymbolicPrinter *getPlan9SymbolicPrinter() {
    return nullptr;
  }
